...
object Matrix {
  ...
  def apply(key: String, index: Int, absRows: Int, absCols: Int, left: Int = 0, up: Int = 0): M =
    toAbsWithMargins(shift(toCheckerboard(getRelSources(key, index)), left, up), absRows, absCols)

  @tailrec
  def shiftX[T: ClassTag](m: Array[T], shift: Int): Array[T] = {
  ...
  /** Each string represents a matrix generated by Veronika Irvine:
    * http://web.uvic.ca/~vmi/papers/interleavedpatterns.html
