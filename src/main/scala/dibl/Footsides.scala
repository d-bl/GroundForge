...
object Footsides {
  /** Tries to create foot sides from fringes of a two-in two out directed graph representing a bobbin lace pair diagram.
    *
    * {{{
    * ::v v::
    * > o o <
    ...
    * ::^ ^::
    * }}}
    * In above ascii art each `o` represent either an unused node or a stitch with two incoming pairs and two outgoing pairs.
    ...
    *
    * @param abs A Matrix, with cells representing nodes of the graph.
    *            The coordinates of a cell represent the target of a link,
    *            each cell contains the coordinates of the sources of links.
    *            The left margin may get additional links to create `K` shaped nodes,
    *            idem but mirrored for the right margin.
    */
  def createFootsides (abs: M):Unit = {
    ...
    val nrOfLinks: Array[Array[Int]] = countLinks(abs)
