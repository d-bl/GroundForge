...
object BenchMark {
  def main(args: Array[String]): Unit = {
    ...
    val loopingPairDiagrams = Seq(
      "gw-H10", "gw-B14", "gw-E14", "gw-H14", "gw-H14a",
      "4x4_201", "4x3_91", "3x4_467", "4x4_195",
    )
    (PatternQueries.gwPatterns ++ PatternQueries.maePatterns)
      .foreach { case (name, q) =>
        println("\n\n\n")
        Try(createSvgFile(s"$dir/$name.svg", TopoLink.fromUrlQuery(q)))
        if (q.matches(".*=[ctlrp]+&.*") && !loopingPairDiagrams.contains(name)) {
          val q3 = s"?$name&" + PairParams(q).toString
          Try(createSvgFile(s"$dir/$name-pair.svg", TopoLink.fromUrlQuery(q3)))
        }
      }
